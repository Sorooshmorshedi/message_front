          ...
          <v-col cols="12">
            ...
            >
              <v-textarea
                ...
                row-height="15"
              ></v-textarea>
              <v-card-subtitle>group image</v-card-subtitle>
              <v-card-text>
...
export default {
  ...
  methods: {
    ...
    CreateChannel() {
      ...
      this.$axios.$put('http://127.0.0.1:8000/api/groupE/' + this.id + '/', this.data)
        .then(response => {
          console.log(response)
          window.alert('group edit')
          window.location.href = "http://127.0.0.1:3000/message/group/" + this.slug + '/?id=' + response.id
        }).catch(response => {
